...
class DepthVideo:
    def __init__(self, image_size=[480, 640], buffer=1024, stereo=False, device="cuda:0"):
        ...
        self.disps_embed = torch.zeros(buffer, c, 2, ht // 8, wd // 8, dtype=torch.half, device="cuda").share_memory_()
        self.disps_embed1 = torch.zeros(buffer, c, 256, ht // 8, wd // 8, dtype=torch.half, device="cuda").share_memory_()
    ...
    def __item_setter(self, index, item):
        ...
        if len(item) > 9:
            self.disps_embed[index] = item[9]

        if len(item) > 10:
            self.disps_embed1[index] = item[10]
    ...
    def __getitem__(self, index):
        ...
        with self.get_lock():
            ...
            item = (
                ...
                self.nets[index],
                self.inps[index],
                self.disps_embed[index],
                self.disps_embed1[index])
