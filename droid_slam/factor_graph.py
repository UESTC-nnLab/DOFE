...
from modules.corr_depth  import CorrBlock_withDepth, AltCorrBlock

class FactorGraph:
    def __init__(self, video, update_op, crossGuide, fftCorr, device="cuda:0", corr_impl="volume", max_factors=-1, upsample=False):
        self.video = video
    ...
    def add_factors(self, ii, jj, remove=False):
        ...
        if self.corr_impl == "volume":
            ...
            left_max = torch.max(ii)
            right_max = torch.max(jj)
            ...
            left_min = torch.min(ii)
            right_min = torch.min(jj)
            ...
            global_corrs = self.fftCorr(self.video.fmaps[index].permute(1,0,2,3,4).squeeze(dim=0), self.video.disps_embed1[index].permute(1,0,2,3,4).squeeze(dim=0), f_ii, f_jj)
            self.global_corrs = global_corrs if self.global_corrs is None else torch.cat([self.global_corrs, global_corrs],dim=0)

            disps_embed2 = self.video.disps_embed[jj, c].to(self.device)

            corr = CorrBlock_withDepth(fmap1, fmap2, disps_embed2)
    ...
    def rm_factors(self, mask, store=False):
        ...
        if self.corr_impl == "volume":
            ...
            self.global_corrs = self.global_corrs[~mask]
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        corr0, disps_corr = self.corr(coords1)  # disps_corr

        corr = self.crossGuide(corr0.squeeze(dim=0), disps_corr.squeeze(dim=0),isVisual=True)
        corr = corr.unsqueeze(dim=0)

        self.net, delta, weight, damping, upmask = \
            self.update_op(self.net, self.inp, corr, motn, self.global_corrs, self.ii, self.jj)
